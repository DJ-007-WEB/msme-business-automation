...
// Use relaxed typing (any) for tooltip content to avoid Recharts type mismatches.
const ChartTooltipContent = React.forwardRef<any, any>((props: any, ref) => {
  const {
    active,
    payload,
    className,
...
const ChartLegendContent = React.forwardRef<any, any>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  const { config } = useChart();

  const list = payload as any[] | undefined;
